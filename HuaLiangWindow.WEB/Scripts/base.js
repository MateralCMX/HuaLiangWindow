...
(function (HuaLiangWindow) {
    var Common = (function () {
            ...
            if (!MTMa.IsNullOrUndefined(PageInfo)) {
                ...
                if (pageM.PagingIndex <= 1) {
                    ...
                    MDMa.AddEvent(FirstBtnA, "click", function (e) {
                        ...
                        HuaLiangWindow.common.PagingM.PageModel.PagingIndex = index;
                        getList();
                    });
                ...
                /*绑定数字*/
                var Count = 0;
                var index = pageM.PagingIndex - Math.round((HuaLiangWindow.common.PagingM.PageShowBtnNum - 1) / 2);
                for (; index <= pageM.PagingCount && Count < HuaLiangWindow.common.PagingM.PageShowBtnNum; index++) {
                    if (index >= 1) {
                ...
                HuaLiangWindow.common.PagingM.PageModel = pageM;
            }
        };
        /**
         * 保存查询条件信息
        ...
        Common.prototype.InputInvalidEvent_Invalid = function (e, setting) {
            ...
            if (inputElement.validity.valueMissing) {
                HuaLiangWindow.common.SetInputErrorMessage(inputElement, options.Required);
            }
            else if (inputElement.validity.patternMismatch) {
                HuaLiangWindow.common.SetInputErrorMessage(inputElement, options.Pattern);
            }
            else if (inputElement.validity.rangeUnderflow) {
                HuaLiangWindow.common.SetInputErrorMessage(inputElement, options.Min);
            }
            else {
                HuaLiangWindow.common.SetInputErrorMessage(inputElement, options.Defult);
            }
        };
        /**
         * 设置错误信息
                ...
                EFun = function (resM, xhr, state) {
                    switch (state) {
                        ...
                        case 401:
                            ...
                            HuaLiangWindow.common.GoToPage("Login", null, true);
                            break;
                        case 403:
                            HuaLiangWindow.common.ShowMessageBox("您的权限不足。");
                            break;
                        case 404:
                            HuaLiangWindow.common.ShowMessageBox("未能找到该资源。");
                            break;
                        default:
                            HuaLiangWindow.common.ShowMessageBox(resM["Message"] != null ? resM["Message"] : "应用程序出错了");
                            break;
                    }
                };
            }
        ...
        Common.prototype.IsLogin = function (gotoLogin) {
            ...
            var userInfo = HuaLiangWindow.common.GetLoginUserInfo();
            if (MTMa.IsNullOrUndefined(userInfo)) {
                if (gotoLogin) {
                    HuaLiangWindow.common.GoToPage("Login", ["from=" + encodeURIComponent(window.location.pathname + window.location.search)], true);
                    return false;
                }
                else {
                    return false;
     ...
     * 应用程序设置模型
     */
    var ApplicationSettingModel = (function () {
        function ApplicationSettingModel() {
            /*域名*/
            this._domainName = "http://www.lwmcdz.com/";
            /*调试域名*/
            this._deBugDomainName = "http://localhost:63699/";
            /*保存用户的值*/
     ...
     * 验证模型
     */
    var InvalidOptionsModel = (function () {
    ...
    HuaLiangWindow.InvalidOptionsModel = InvalidOptionsModel;
    /**
     * 登录用户模型
     */
    ...
    HuaLiangWindow.LoginUserModel = LoginUserModel;
    /**
     * 登录用户模型
     */
    var MPagingModel = (function () {
        function MPagingModel() {
        }
        return MPagingModel;
    }());
    HuaLiangWindow.MPagingModel = MPagingModel;
    /**
     * 公共处理对象
     */
    HuaLiangWindow.common = new Common();
